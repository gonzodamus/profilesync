...
function PS:CreateMainFrame()
    ...
    -- Title (robust across game versions)
    if frame.Title then
        frame.Title:SetText("ProfileSync")
    else
        local title = frame:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
        title:SetPoint("TOPLEFT", 20, -16)
        title:SetText("ProfileSync")
        frame._TitleText = title
    end
...
function PS:CreateContent()
    ...
    if untestedCheckbox.Text then
        untestedCheckbox.Text:SetText("Allow untested addon versions")
    elseif untestedCheckbox.text then
        untestedCheckbox.text:SetText("Allow untested addon versions")
    end
    ...
    UIDropDownMenu_Initialize(dropdown, function(self, level)
        if level == 1 then
            ...
            for _, profile in ipairs(profiles) do
                ...
                info2.value = p
                info2.func = function()
                    PS:SaveAddonProfile(addonName, p)
                    UIDropDownMenu_SetText(dropdown, p)
                end
